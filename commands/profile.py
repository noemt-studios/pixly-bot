...
from util.views import ProfileCommandProfileSelector
...
class Profile(Cog):
    ...
    @slash_command(
        name="profile",
        description="Get the profile stats of a player",
    ...
    @option(
        name="profile",
        description="The profile to get the stats of",
        required=False,
        type=str,
        autocomplete=get_profiles
    )
    async def profile(self, ctx:discord.ApplicationContext, name: str, profile: str = "selected"):
        await ctx.defer()
