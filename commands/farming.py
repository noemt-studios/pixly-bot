...
from util.views import FarmingProfileSelector
...
class Farming(Cog):
    ...
    @slash_command(
        name="farming",
        description="Get the farming stats of a player",
        integration_types={discord.IntegrationType.user_install, discord.IntegrationType.guild_install},
    )
    @option(
        name="name",
        description="The player to get the farming stats of",
        required=False,
        type=str,
    ...
    @option(
        name="profile",
        description="The profile to get the farming stats of",
        required=False,
        type=str,
        autocomplete=get_profiles
    )
    async def farming(self, ctx:discord.ApplicationContext, name: str, profile: str = "selected"):
        await ctx.defer()
